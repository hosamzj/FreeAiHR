...
export default function ResumesPage() {
  ...
  // Screen notes state
  const [showScreenNotesDialog, setShowScreenNotesDialog] = useState(false);
  const [screenNotesCandidate, setScreenNotesCandidate] = useState<{ id: string; name: string } | null>(null);
  const [screenNotes, setScreenNotes] = useState('');
  const [hrNotesPopover, setHrNotesPopover] = useState<string | null>(null);
  ...
  const handlePassScreen = useCallback((candidateId: string, candidateName: string) => {
    setScreenNotesCandidate({ id: candidateId, name: candidateName });
    setScreenNotes('');
    setShowScreenNotesDialog(true);
  }, []);

  const confirmPassScreen = useCallback(async () => {
    if (!screenNotesCandidate) return;
    const candidateId = screenNotesCandidate.id;
    const candidateName = screenNotesCandidate.name;
    setActionLoading(candidateId);
    setShowScreenNotesDialog(false);
    try {
      // Determine next status based on current status
      const candidate = candidates.find(c => c.id === candidateId);
      const nextStatus = candidate?.status === 'screening' ? 'interviewing' : 'screening';
      // Save notes and update status via API
      await fetch(`/api/candidates/${candidateId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ status: nextStatus, notes: screenNotes }),
      });
      // Update local state
      setCandidates(prev => prev.map(c => 
        c.id === candidateId ? { ...c, status: nextStatus as typeof nextStatus, notes: screenNotes } : c
      ));
      ...
      setScreenNotesCandidate(null);
    }
  }, [screenNotesCandidate, screenNotes, candidates]);
  ...
  return (
    <div className="space-y-4 md:space-y-5">
                ...
                {candidate.notes && (
                  <div className="mt-3 rounded-lg border border-amber-500/20 bg-amber-500/5 p-3">
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <p className="text-[11px] md:text-xs font-medium text-amber-400">HR 注释</p>
                    </div>
                    <p className="text-[11px] md:text-xs leading-relaxed text-slate-300">{candidate.notes}</p>
                  </div>
                )}
                <div>
                    ...
                    <>
                      {candidate.notes && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setHrNotesPopover(hrNotesPopover === candidate.id ? null : candidate.id);
                          }}
                          className="flex h-7 md:h-8 items-center gap-1 rounded-lg border border-amber-500/20 px-2.5 md:px-3 text-[11px] md:text-xs text-amber-400 hover:bg-amber-500/10 cursor-pointer transition-colors relative"
                        >
                          <MessageSquare className="h-3 w-3" /> HR注释
                          {hrNotesPopover === candidate.id && (
                            <div className="absolute bottom-full left-0 mb-2 w-56 rounded-lg border border-amber-500/30 bg-[#1a2236] p-3 shadow-lg z-50">
                              <div className="flex items-center gap-2 mb-1.5">
                                <MessageSquare className="h-3.5 w-3.5 text-amber-400" />
                                <p className="text-[10px] font-medium text-amber-400">HR 注释</p>
                              </div>
                              <p className="text-[11px] text-slate-300 leading-relaxed">{candidate.notes}</p>
                            </div>
                          )}
                        </button>
                      )}
      ...
      {/* Screen Notes Modal - HR输入筛选注释 */}
      <Modal isOpen={showScreenNotesDialog} onClose={() => setShowScreenNotesDialog(false)} title="通过筛选 - 输入注释">
        <div className="space-y-4">
          {screenNotesCandidate && (
            <div className="flex items-center gap-3 rounded-lg bg-sky-500/5 border border-sky-500/20 p-3">
              <p className="text-sm text-slate-300">
                为 <span className="text-sky-400 font-medium">{screenNotesCandidate.name}</span> 添加筛选注释
              </p>
            </div>
          )}
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1.5">HR 注释</label>
            <textarea
              rows={4}
              value={screenNotes}
              onChange={(e) => setScreenNotes(e.target.value)}
              placeholder="输入筛选意见、候选人亮点、推荐理由等，面试主管可以看到此注释..."
              className="w-full rounded-lg border border-[#1e293b] bg-[#0a0e1a] px-3 py-2 text-sm text-slate-300 placeholder:text-slate-600 focus:border-sky-500/50 focus:outline-none resize-none"
            />
          </div>
          <div className="flex gap-2 pt-2">
            <button
              onClick={confirmPassScreen}
              disabled={actionLoading === screenNotesCandidate?.id}
              className="flex-1 h-9 rounded-lg bg-sky-500 text-sm font-medium text-white hover:bg-sky-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {actionLoading === screenNotesCandidate?.id ? '处理中...' : '确认通过'}
            </button>
            <button
              onClick={() => { setShowScreenNotesDialog(false); setScreenNotesCandidate(null); }}
              disabled={actionLoading === screenNotesCandidate?.id}
