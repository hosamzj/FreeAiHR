...
export async function POST(request: NextRequest) {
  try {
    ...
    const demoAccounts: Record<string, { name: string; role: string; password: string }> = {
      'admin@recruit.ai': { name: '系统管理员', role: 'admin', password: 'Admin@123' },
      'hr@recruit.ai': { name: '招聘经理', role: 'hr_manager', password: 'Hr@12345' },
      'interviewer@recruit.ai': { name: '面试官', role: 'interviewer', password: 'Test@1234' },
    };

    if (isDemoMode && demoAccounts[email]) {
      const demoUser = demoAccounts[email];
      if (password !== demoUser.password) {
        return unauthorized('邮箱或密码错误');
      }
      const token = generateToken({ userId: email, email: email, role: demoUser.role, name: demoUser.name });
      const response = NextResponse.json({ code: 0, data: {
        needPasswordChange: false,
        user: { id: email, email: email, name: demoUser.name, role: demoUser.role },
      }, message: 'success' });
